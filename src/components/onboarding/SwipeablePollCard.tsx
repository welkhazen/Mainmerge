...
export function SwipeablePollCard({
  ...
  const noOption = options.find((option) => option.trim().toLowerCase() === "no") ?? options[1] ?? options[0];
  ...
  const voteFromSwipeDirection = (direction: "left" | "right") => {
    ...
    const selected = direction === "right" ? yesOption : noOption;
    if (!selected) return;
      ...
      <div className="rounded-[1.25rem] border border-[#4c3a12] bg-[linear-gradient(180deg,#151515_0%,#0d0d0d_100%)] p-4">
        ...
        <div className="relative mt-5">
          ...
          >
            ...
            <p className="mt-3 text-center text-xs text-white/45">Swipe right for Yes, left for No</p>

            <div className="mt-6 grid grid-cols-2 gap-3">
              {[yesOption, noOption].map((option, index) => {
                const isYes = index === 0;
        ...
        <section className="mt-4 rounded-xl border border-[#4f4f4f] bg-[#101010] p-3">
          ...
          <div className="mt-3">
            {updatedComments.length === 0 ? <p className="text-xs text-white/45">No comments yet. Be the first.</p> : null}
          </div>
        </section>
      </div>
    </div>
