...
3. Alternative for plain HTML pages (non-React fallback): use the PostHog snippet in `index.html` instead of the React provider.
...
> Better option for this repo: keep using the React provider path in `src/main.tsx` so hooks (`usePostHog`) and app-level tracking stay consistent.
